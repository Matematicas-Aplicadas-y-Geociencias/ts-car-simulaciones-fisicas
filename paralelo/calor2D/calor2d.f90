Program Calor2D
  ...
  bucle_iteraciones: do iter = 1, itermax
     ...
     !
     !--------------------------------------------------------------------------
     ! Paralelizamos el barrido en la dirección y en bandas 
     ! compuestas por grupos de líneas, observamos que si usamos pocas líneas
     ! tenemos una acelaración pobre o asuente 
     !
     !$omp parallel do default(none)  &
     !$omp shared(tt, deltax, deltay, cfx) private(ax, bx, cx, rx, tx, ii)
     ...
     !
     !$omp parallel do default(none)  &
     !$omp shared(tt, deltax, deltay, cfy) private(ay, by, cy, ry, ty, jj)
     ...
     !
     residuo = 0.d0
     !$omp parallel reduction(+:residuo) shared(tt) 
     !$omp do private(jj)
     do ii = 1, nx
        do jj = 1, ny
  ...
  call residuo_temp(tt, nx, ny, lx, ly, residuo_code)
  write(*, *) "Valor del residuo ", residuo_code
