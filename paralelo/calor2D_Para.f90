Program Calor2D
  ...
  bucle_iteraciones: do iter = 1, itermax
     ...
     !
     tt(:,:,2) = tt(:,:,1)
     !$omp parallel do default(none) shared(deltax,deltay,tt,cfx) private(ax,bx,cx,rx,tx) 
     barrido_y: do jj = 2, ny-1
     !Es posible combinar directivas de omp
     !por ejemplo si la región es sólo para un do, sin embargo, el hacer paralelismo de grano fino en este caso empeora el rendimiento del código, tal vez valga más la pena hacerlo de grano grueso sobre el bucle barrido
     	! $omp parallel do default(none) &
     	! $omp shared(ax,bx,cx,rx,tt,cfx,deltax,deltay,jj) &
     	! $omp private(ii)
     	!se elije ii privada para que cada hilo tenga un 		cacho de bucle
     ...
     !$omp parallel do default(none) shared(deltax, deltay,tt,cfy) private(ay,by,cy,ry,ty) 
     barrido_x: do ii = 2, nx-1
	! $omp parallel do default(none) &
     	! $omp shared(ay,by,cy,ry,tt,cfy,deltax,deltay,ii) &
     	! $omp private(jj)
     ...
     !
     residuo = 0.d0
     ! $omp parallel reduction(+:residuo)
     ! $omp do
     do ii = 1, nx
        do jj = 1, ny
