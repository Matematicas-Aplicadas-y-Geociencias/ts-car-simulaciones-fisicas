Program Calor2D
  ...
  !                                          ! matriz tridiagonal sobredimensionada
  !
  double precision :: frac1,frac2 ! Esta definición la uso para hacer mi chanchullo con las operaciones que se repiten muchísmo.
  ...
  bucle_iteraciones: do iter = 1, itermax
     ...
     !
     !$omp parallel do default(none) private(ii,jj,ax,bx,cx,rx,tx) shared(frac1,frac2,tt,deltax,deltay,cfx)
     !OJO: Yo sé que no es necesario hacer private(ii,jj) pero me gusta hacerlo, me ayuda a tener claridad.
     ...
     barrido_y: do jj = 2, ny-1
        ensambla_tri_x: do ii = 2, nx-1
           ...
           cx(ii) = frac1
           rx(ii) =-frac2*tt(ii,jj-1,2)-frac2*tt(ii,jj+1,2)  !---> uso tt(:,:,2) para evitarme problemas 
     ...
     frac1=1.d0/(deltax*deltax) !creo que no necesito redefiniarlas, pero just in case.
     frac2=1.d0/(deltay*deltay) 
     !
     !$omp parallel do default(none) private(ii,jj,ay,by,cy,ry,ty) shared(frac1,frac2,tt,deltax,deltay,cfy)
     ...
     barrido_x: do ii = 2, nx-1
        ...
        ensambla_tri_y: do jj = 2, ny-1
           ...
           cy(jj) = frac2 
           ry(jj) =-frac1*tt(ii-1,jj,2)-frac1*tt(ii+1,jj,2)
     ...
     !
     residuo = 0.d0
     !$omp parallel do default(none) private(ii,jj) shared(tt) reduction(+:residuo)
     do ii = 1, nx
        do jj = 1, ny
