Program Calor2D
  ...
  bucle_iteraciones: do iter = 1, itermax
     ...
     barrido_y: do jj = 2, ny-1
        ...
        call tri(aa((jj-1)*nx+1 : jj*nx), bb((jj-1)*nx+1 : jj*nx),cc((jj-1)*nx+1 : jj*nx), rr((jj-1)*nx+1 : jj*nx), tt(1:nx,jj,1), nx)
        !
     ...
     barrido_x: do ii = 2, nx-1
        ...
        call tri(aa((ii-1)*ny+1 : ii*ny), bb((ii-1)*ny+1 : ii*ny),cc((ii-1)*ny+1 : ii*ny), rr((ii-1)*ny+1 : ii*ny), ty((ii-1)*ny+1 : ii*ny), ny)
        ! Actualizar la temperatura de la placa
        !
        do jj = 1, ny
           
           tt(ii,jj,1) = ty(jj+ ny*(ii-1))
